...
const HELP_DETAILS: Record<string, HelpItem[]> = {
  ...
  '/familia': [
    { title: 'Miembros', text: 'Nombre, tipo (Adulto, Niño/a, Bebé, Invitado/a), color y foto de cada uno; arrastra para reordenar la lista. Al añadir el 2º miembro de la familia se pregunta una sola vez si lleváis las cuentas Compartidas o Separadas (ver Configuración → Modo de cuentas) — se puede cambiar después cuando quieras.' },
...
const SETTINGS_ENTRY = {
  ...
  details: [
    ...
    { title: 'Modo de cuentas', text: 'Compartidas (por defecto): todo el mundo ve los movimientos y saldos de todos, como hasta ahora. Separadas: cada uno ve solo los suyos en Banco/Movimientos/Estadísticas — ni el admin ve los de otro miembro, aunque sí puede desconectar cualquier cuenta — y aparece un interruptor "Individual/Común" en Movimientos y Presupuesto(s) para lo que se comparta a propósito. Se pregunta la primera vez que la familia pasa de 1 a 2 miembros, y se puede cambiar aquí cuando haga falta.' },
    { title: 'Cuentas bancarias', text: 'Conectar un banco nuevo, desconectarlo o decir de quién es cada cuenta ("🏠 Común" si es de toda la familia). En modo Separadas, la tarjeta de una cuenta que no es tuya ni Común muestra 🔒 en vez del saldo. Por normativa (PSD2) cada banco pide renovar el permiso cada 90 días — se ve como "válido hasta" junto al nombre del banco; cuando se acerque esa fecha, vuelve aquí y conecta el mismo banco otra vez para que la sincronización no se corte. Le pasa igual a cualquier familia y a cualquier app de banca abierta, no es un fallo. Para no tener que estar pendiente, al conectar un banco se apuntan solos dos avisos en el Calendario, uno una semana antes de esa fecha y otro un día antes.' },
