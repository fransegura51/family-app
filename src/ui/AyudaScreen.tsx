...
const SETTINGS_ENTRY = {
  ...
  details: [
    ...
    { title: 'Cuentas bancarias', text: 'Conectar un banco nuevo, desconectarlo o decir de quién es cada cuenta ("🏠 Común" si es de toda la familia) — al conectar una cuenta se te asigna a ti automáticamente, sin quedarse como Común por defecto. En modo Separadas, la tarjeta de una cuenta que no es tuya ni Común muestra 🔒 en vez del saldo, y solo el dueño actual de una cuenta (o el administrador) puede cambiarle la etiqueta — nadie puede "quitarle" una cuenta a otro poniéndose a sí mismo como dueño. Por normativa (PSD2) cada banco pide renovar el permiso cada 90 días — se ve como "válido hasta" junto al nombre del banco; cuando se acerque esa fecha, vuelve aquí y conecta el mismo banco otra vez para que la sincronización no se corte. Le pasa igual a cualquier familia y a cualquier app de banca abierta, no es un fallo. Para no tener que estar pendiente, al conectar un banco se apuntan solos dos avisos en el Calendario, uno una semana antes de esa fecha y otro un día antes.' },
