...
const HELP_DETAILS: Record<string, HelpItem[]> = {
  ...
  '/dinero': [
    { title: 'Resumen', text: 'Ingresos, gastos y ahorro del mes, más las "Conclusiones de Pepa": en su bocadillo se ve una frase a la vez señalando algo distinto (la categoría que más ha crecido, la tienda más frecuente...) — cambia sola cada vez que abres la pantalla, o deslizando con el dedo sobre la imagen para ver las demás. Cuando la conclusión se apoya en un criterio propio de la app (tasa de ahorro, Fijo/Variable, Debo/Necesito/Quiero, comparación con el periodo anterior), debajo de la imagen aparece un cuadro "💡" explicando qué significa y por qué importa. Su filtro "📅 Fecha" (como en el resto de Economía) incluye "Mes contable" y "Mes real" — ver Presupuesto Generales para la diferencia. Debajo de las tarjetas de "Mis cuentas", "⚙️ Configuración cuentas" lleva directo a conectar/desconectar bancos o decir de quién es cada cuenta, sin tener que buscarlo en ☰ Menú.' },
    { title: 'Estadísticas', text: 'Debajo de "Mis cuentas" está "Tendencia del saldo" (solo se ve aquí), con su propio filtro "📅 Fecha" (Mes contable/Mes real incluidos): la evolución del saldo día a día, reconstruida a partir de los movimientos guardados, con eje de euros y de fechas — viendo "Todas" se dibuja como franjas apiladas, una por cuenta y con su color (el del miembro asignado, o gris "🏠 Común" si no tiene dueño). Debajo, los dónuts por categoría, etiqueta, Debo/Necesito/Quiero y Fijo/Variable (con su propio filtro "📅 Fecha" también) — toca una porción para ver justo esos movimientos. Cada categoría tiene siempre el mismo color (sus subcategorías, el mismo tono en distinta intensidad), agrupadas en la leyenda (a la izquierda del dónut) por categoría padre. "Evolución temporal" (últimos 6 meses) respeta el día de inicio del mes contable que hayas puesto en Configuración.' },
    { title: 'Movimientos', text: 'Todos los gastos e ingresos del periodo, editables uno a uno — con el mismo filtro "📅 Fecha" desplegable que el resto de Economía (Mes contable/Mes real incluidos), en vez de la navegación por mes de antes. Junto a la categoría, el avatar (o "🏠 Común") dice de qué cuenta bancaria enlazada es cada movimiento — igual que en Banco.' },
    { title: 'Presupuesto Generales', text: 'Un solo filtro de fecha (📅 arriba de todo) para toda la pestaña — incluye "Mes contable" (respeta el día de inicio que hayas puesto en Configuración) y "Mes real" (siempre 1 a fin de mes, aunque tengas otro día configurado). "Resumen": si NO tienes ninguna cuenta bancaria enlazada, "Ingresos" es lo que apuntes tú a mano con "+ Añadir ingreso"; en cuanto enlazas una cuenta, ese botón desaparece y el importe pasa a salir solo de lo que de verdad haya entrado en tus cuentas — apuntar un ingreso a mano con el banco ya enlazado se contaría dos veces y descuadraría con Movimientos, así que un ingreso real (nómina, etc.) se registra directamente en Movimientos, no aquí. Los ingresos que ya tuvieras apuntados a mano siguen ahí para poder borrarlos, pero dejan de sumar. El Presupuesto del mes contable y el Gastado van justo debajo para comparar de un vistazo; la lista de categorías está agrupada por categoría padre, ordenada de mayor a menor gasto — toca el nombre para desplegar sus subcategorías. Debajo, "Reparto del gasto por categoría" es el mismo dónut por subcategoría. Al final, "Presupuestos": el del mes contable en curso siempre a la vista arriba, el formulario para crear uno nuevo justo debajo (avisa si ya hay uno igual para ese mes), e "Historial" para consultar los de meses anteriores.' },
    { title: 'Banco', text: 'Los movimientos se traen y concilian solos varias veces al día, o pulsa "🔄 Sincronizar movimientos" para forzarlo (el desplegable de al lado amplía cuánto histórico pedir). Su filtro "📅 Fecha" incluye Mes contable/Mes real, igual que el resto de Economía. Conectar un banco nuevo, desconectarlo o decir de quién es cada cuenta se hace desde "⚙️ Configuración cuentas" (arriba, junto a "Mis cuentas") — lleva a ☰ Menú → Configuración → 🏦 Cuentas bancarias. Viendo todas las cuentas mezcladas, el avatar (o "🏠 Común") junto a cada movimiento dice de quién es la cuenta, sin tener que filtrar una a una.' },
