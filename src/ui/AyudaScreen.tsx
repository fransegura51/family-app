...
const HELP_DETAILS: Record<string, HelpItem[]> = {
  ...
  '/compras': [
    ...
    { title: 'Estadística compras', text: '"Total Registrado en Compras" suma Alimentación y Otros, con el importe de cada uno y qué porcentaje representa tanto de las compras como del gasto total de Economía en ese periodo — filtra por Hoy/Esta semana/Mes contable/Mes real/Este año/Rango con el mismo botón "📅 Fecha" que Presupuesto Generales. "Otros" cuenta los tickets/pedidos que no sean de Alimentación más cualquier gasto metido a mano en Economía bajo "Compras y familia" (Ropa, Niños, Casa y jardín, Tecnología, Mascotas, Regalos...). Cada importe lleva su "Ver movimientos →", que lleva directo a Movimientos (en Economía) ya filtrado con exactamente esos mismos gastos. Debajo, "Reparto del gasto por tienda" tiene arriba dos botones, Alimentación/Otros, para ver el mismo dónut con los gastos de uno u otro — toca una porción para ir a Movimientos filtrado igual. Luego dos dónuts más, uno por Alimentación y otro por Otros, que clasifican cada producto de los tickets por su nombre (no por la categoría del gasto entero, que casi siempre es la misma para todo un ticket): el de Alimentación por tipo de alimento (Verdura, Fruta, Carne, Pescado, Lácteos y huevos, Panadería, Bebidas, Congelados, Snacks, Despensa...) con clasificación automática por palabras clave (no perfecta; lo que no encaje cae en "Otros alimentos"), y el de Otros por la Clasificación que hayas elegido a mano para cada producto en el ⚙️ de Historial de precios (sin adivinador, así que lo no clasificado cae en "Sin clasificar"). Un mismo ticket puede repartirse entre los dos dónuts a la vez si mezcla productos de los dos tipos (p. ej. un ticket de Mercadona con un producto marcado como Otros) — por eso ninguno de los dos lleva enlace a movimientos, un ticket no tiene un único gasto 1:1 con "Verdura" o "Ropa". Un gasto de banco sin ningún ticket detrás (una tarjeta cobrada directamente en una tienda de ropa, por ejemplo) cuenta entero como "Sin clasificar" en su dónut, salvo que le pongas tú la Clasificación a mano desde Movimientos (ver Economía → Movimientos, editar el gasto) — así el total de cada dónut cuadra siempre con el de "Reparto del gasto por tienda", tenga ticket detrás o no. Debajo de cada dónut, una lista con cada tipo/clase y su % del total — toca uno para desplegar los productos concretos que lo forman, con su importe y el % que representan dentro de ese tipo. En todos los dónuts de la app, cada porción tiene siempre un color distinto de las demás, por muchas que haya (tiendas, categorías, tipos...); nunca se repite uno ya usado.' },
  ],
  '/alimentacion': [
