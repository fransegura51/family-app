-- Bug real en producción, encontrado al dar de alta una cuenta de
-- pruebas: crear una familia nueva fallaba SIEMPRE con "DELETE
-- requires a WHERE clause". Causa exacta (visto en postgres_logs):
-- `delete from tmp_create_family_cat_map;` dentro de create_family
-- (0117_seed_new_families_from_template.sql) — Supabase bloquea, para
-- la conexión de PostgREST, cualquier DELETE sin condición, y esa
-- línea nunca la tenía. Es además innecesaria: la tabla temporal se
-- crea vacía en cada llamada (ON COMMIT DROP + transacción nueva por
-- petición), así que basta con quitar el DELETE, no con darle una
-- condición postiza.
