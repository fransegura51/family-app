...
const HELP_DETAILS: Record<string, HelpItem[]> = {
  ...
  '/dinero': [
    ...
    { title: 'Estadísticas', text: 'Debajo de "Mis cuentas" está "Tendencia del saldo" (solo se ve aquí): la evolución del saldo día a día, reconstruida a partir de los movimientos guardados, con eje de euros y de fechas — viendo "Todas" se dibuja como franjas apiladas, una por cuenta y con su color (el del miembro asignado, o gris "🏠 Común" si no tiene dueño). Debajo, los dónuts por categoría, etiqueta, Debo/Necesito/Quiero y Fijo/Variable — toca una porción para ver justo esos movimientos. Cada categoría tiene siempre el mismo color (sus subcategorías, el mismo tono más claro u oscuro), y la leyenda va a la derecha del dónut. "Evolución temporal" (últimos 6 meses) respeta el día de inicio del mes contable que hayas puesto en Configuración.' },
    { title: 'Movimientos', text: 'Todos los gastos e ingresos del mes, editables uno a uno.' },
    { title: 'Presupuesto Generales', text: '"Presupuesto total del mes" (arriba de todo) marca el tope y cuánto lleváis gastado, con su barra de %. "Resumen" es aparte: Ingresos son solo lo que apuntes tú a mano (no lo calcula de tus cuentas bancarias), con el Presupuesto del mes y el Gastado justo debajo para comparar de un vistazo; la lista de categorías está agrupada por categoría padre, ordenada de mayor a menor gasto — toca el nombre para desplegar sus subcategorías. Más abajo, "Reparto del gasto por categoría" es el mismo dónut por subcategoría, con el mismo color por categoría que en Estadísticas. Todo respeta el día de inicio del mes contable que hayas puesto en Configuración.' },
